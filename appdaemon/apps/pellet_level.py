...
class PelletLevel(hass.Hass):
    """
    Build a B/W mask (pellets bright -> white), then detect fill level by the
    first/top row that has a contiguous white run >= min_run_px for
    >= min_run_rows consecutive rows. That row -> fill fraction.

    Sensor state: sacks (0–12, one decimal)
    Attributes:
      - PercentageOfWhitePixels  (raw, unfiltered % by area)
      - FillLevelPercentage      (from level-line, 0–100)
    """
    ...
    def initialize(self):
        ...
        self.mask_output_path: Optional[str] = self.args.get("mask_output_path")  # optional
        ...
        # NEW: binary closing to remove black specks in white area
        self.close_size: int = int(self.args.get("close_size", 3))                 # odd (3,5,7). 0=off

        # Level-line detection
        self.min_run_px: int     = int(self.args.get("min_run_px", 25))
        self.min_run_rows: int   = int(self.args.get("min_run_rows", 3))  # consecutive rows

        # Calibration: raw fill fraction (0..1) -> sacks (0..12)
    ...
    def _process_once(self, *_):
        try:
            ...
            # Morphological closing (fill black pinholes in white)
            if self.close_size and self.close_size >= 3 and self.close_size % 2 == 1:
                # close = dilate then erode; emulate with Max then Min filters
            ...
            # --- Raw (unfiltered) white % for attribute ---
            hist = mask.histogram()
            white = hist[255]
            total = w * h
            raw_white_pct = round((white / total) * 100.0, 1) if total else 0.0

            # --- Level-line detection ---
            y_level = self._find_first_row_with_count(mask, self.min_run_px, self.min_run_rows)
            if y_level is None:
                # Fallback: use raw area fraction if no stable line found
                fill_frac = (white / total) if total else 0.0
            else:
                # top = 0 (full), bottom = h-1 (empty)
                fill_frac = 1.0 - (y_level / float(h))

            # Clamp
            fill_frac = max(0.0, min(1.0, fill_frac))

            # Map to sacks via calibration
    ...
    # ---- level-line scanner ----
    def _find_first_row_with_count(self, mask, min_white_px: int, min_rows: int):
        """Return topmost y where total white pixels in a row >= min_white_px
        for >= min_rows consecutive rows. mask is L (0/255)."""
        w, h = mask.size
        pixels = mask.load()
        consec = 0
        for y in range(h):
            # count total whites in the row (no contiguity requirement)
            white_count = 0
            for x in range(w):
                if pixels[x, y] == 255:
                    white_count += 1
